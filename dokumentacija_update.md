...
Atribut id je primarni ključ (PRIMARY KEY) tipa INT AUTO_INCREMENT, jedinstveno identificira svaku članarinu. Atribut tip (VARCHAR(50) NOT NULL UNIQUE) označava naziv članarine (npr. “Osnovna”, “Premium”) i jedinstven je, pa se ne može ponavljati. Atribut cijena (DECIMAL(6,2) NOT NULL CHECK (cijena >= 0)) bilježi iznos u eurima, ne može biti negativan. Atribut trajanje (INT NOT NULL CHECK (trajanje > 0)) govori o duljini članarine u mjesecima, mora biti barem 1. Atribut opis (TEXT) može sadržavati dodatne napomene o pogodnostima ili uvjetima i može ostati prazan.
...
Atribut id je primarni ključ (INT AUTO_INCREMENT) i jedinstveno identificira člana. Atributi ime i prezime (VARCHAR(50) NOT NULL) su obavezni za unos osobnih podataka. Atribut email (VARCHAR(100) NOT NULL UNIQUE) mora biti jedinstven i ne smije biti prazan. Atribut telefon (VARCHAR(20)) može ostati prazan ako se ne unese. Atribut datum_uclanjenja (DATE NOT NULL DEFAULT CURRENT_DATE) automatski pohranjuje datum učlanjenja. Atribut id_clanarina (INT) je strani ključ na clanarina(id), označava vrstu članarine koju član odabere. Atribut aktivan (BOOLEAN DEFAULT TRUE) pokazuje je li članstvo aktivno. Atribut datum_rodjenja (DATE) i spol (ENUM('M','Ž')) mogu ostati prazni. Atribut adresa (VARCHAR(200)) i grad (VARCHAR(50) DEFAULT 'Zagreb') pohranjuju adresu prebivališta.
...
Atribut id je primarni ključ (PRIMARY KEY) tipa INT, s opcijom AUTO_INCREMENT, čime se automatski generira identifikator za svakog trenera.
Atributi ime i prezime su tipa VARCHAR(50) i obavezni su (NOT NULL) jer predstavljaju osnovne podatke trenera.
Atribut specijalizacija označava područje stručnosti trenera (npr. "Rehabilitacija", "Kondicijska priprema") i definiran je kao VARCHAR(100).
Atribut email je VARCHAR(100), a telefon VARCHAR(20). Oba podatka služe za kontaktiranje trenera i mogu biti neobavezni, ovisno o dostupnosti podataka.
...
Atribut id je PRIMARY KEY, INT, automatski se povećava (AUTO_INCREMENT) i koristi se za jedinstvenu identifikaciju svakog tipa treninga.
Atribut naziv je VARCHAR(50), NOT NULL i UNIQUE – definira naziv tipa treninga (npr. "Pilates", "Kondicija") i mora biti jedinstven.
Atribut osnovna_cijena je DECIMAL(6,2), NOT NULL, i predstavlja početnu cijenu treninga u eurima – ne može biti negativna.
Atribut opis je TEXT i koristi se za dodatne informacije o vrsti treninga – nije obavezan.
...
Atribut id je PRIMARY KEY, INT, automatski se povećava (AUTO_INCREMENT) i koristi se za jedinstvenu identifikaciju svakog privatnog treninga.
Atribut id_clana je strani ključ (FOREIGN KEY) koji označava člana koji sudjeluje na treningu – povezan je s clan(id).
Atribut id_trenera je strani ključ koji označava trenera koji vodi trening – povezan je s trener(id).
Atribut id_tip_treninga je strani ključ koji označava vrstu treninga – povezan je s tip_treninga(id).
Atribut datum je DATE i definira kada se trening održava, dok vrijeme (TIME) označava točan početak.
Atribut trajanje je INT i predstavlja trajanje treninga u minutama (maksimalno 180 minuta).
Atribut napomena je TEXT i služi za dodatne informacije, po potrebi.
Atribut status je ENUM ('zakazan', 'održan', 'otkazan') s početnom vrijednosti 'zakazan', i definira stanje treninga.
Atribut cijena je DECIMAL(6,2) i prikazuje konačan iznos koji član plaća za taj trening.
...
Atribut id je primarni ključ (PRIMARY KEY) tipa INT s AUTO_INCREMENT i koristi se za jedinstvenu identifikaciju svakog grupnog treninga.
Atribut naziv opisuje vrstu grupnog treninga (npr. "Pilates", "HIIT") i definiran je kao VARCHAR(100), obavezan za unos.
Atribut id_trenera je strani ključ koji označava trenera koji vodi trening – povezan je s trener(id).
Atribut max_clanova je INT koji označava maksimalan broj članova na treningu, uz ograničenje da mora biti veći od 0 i najviše 30.
Atribut dan_u_tjednu je ENUM koji definira dan kada se trening održava (npr. "Ponedjeljak", "Srijeda").
Atribut vrijeme je TIME i koristi se za zakazivanje točnog termina treninga, s početnom vrijednosti 18:00:00.
Atribut trajanje je INT i predstavlja trajanje treninga u minutama, s početnom vrijednosti 60.
Atribut cijena_po_terminu je DECIMAL(5,2) i označava cijenu po jednom terminu, mora biti veća od 0.
Atribut aktivan je BOOLEAN koji označava je li trening trenutno aktivan (zadana vrijednost je TRUE).
Atribut opis je TEXT i koristi se za dodatne informacije o treningu.
...
Atribut id je primarni ključ (PRIMARY KEY) tipa INT s AUTO_INCREMENT, i koristi se za jedinstvenu identifikaciju svakog zapisa prisutnosti.
Atribut id_clana je strani ključ koji označava člana koji je sudjelovao na treningu – povezan je s clan(id).
Atribut id_grupnog_treninga je strani ključ koji označava grupni trening kojem je član prisustvovao – povezan je s grupni_trening(id).
Atribut datum je tipa DATE i koristi se za označavanje točnog dana prisustva. Ovaj podatak je obavezan (NOT NULL), jer omogućuje razlikovanje višestrukih prisustava na istom treningu.
Atribut prisutan je BOOLEAN koji označava je li član bio prisutan ili ne, s početnom vrijednosti TRUE.
...
Atribut id je primarni ključ (PRIMARY KEY) tipa INT i automatski se generira (AUTO_INCREMENT).
Atribut sifra je VARCHAR(20), mora biti jedinstvena (UNIQUE) i označava inventarnu oznaku opreme (npr. "SPR-001").
Atribut naziv je VARCHAR(100) i opisuje vrstu opreme (npr. "Bench klupa").
Atribut datum_nabave je tipa DATE i obavezan je za praćenje datuma nabave opreme.
Atribut stanje je ENUM s mogućim vrijednostima: 'ispravna', 'u servisu', 'potrebna zamjena dijela', 'neispravna', 'nova' i po defaultu je 'nova', te opisuje trenutno stanje opreme.
Atribut vrijednost je DECIMAL(8,2) i označava financijsku vrijednost opreme, s početnom vrijednošću 0.00.
Atribut proizvodac i model su VARCHAR(50) i služe za unos podataka o proizvođaču i modelu opreme.
Atribut garancija_do je DATE i bilježi do kada vrijedi garancija na opremu.
...
Atribut id je primarni ključ (PRIMARY KEY) tipa INT s AUTO_INCREMENT.
id_clana je strani ključ koji povezuje rezervaciju s članom – referencira clan(id).
id_opreme je strani ključ koji označava rezerviranu spravu – povezan je s oprema(id).
datum je DATE i označava dan rezervacije.
vrijeme_pocetka i vrijeme_zavrsetka su tipa TIME i određuju vremenski raspon rezervacije; vrijeme_zavrsetka mora biti kasnije od vrijeme_pocetka.
Atribut status je ENUM s vrijednostima 'aktivna', 'završena' i 'otkazana', s default vrijednošću 'aktivna'.
napomena je tekstualno polje za dodatne informacije o rezervaciji.

6.10. TABLICA osoblje
Tablica osoblje pohranjuje podatke o zaposlenicima teretane, uključujući trenere, recepcioniste i tehničko osoblje.
Sadrži atribute: id, ime, prezime, uloga, email, telefon, datum_zaposlenja, radno_vrijeme i aktivan.
id je primarni ključ (PRIMARY KEY) tipa INT s AUTO_INCREMENT i jedinstveno identificira svakog zaposlenika.
ime i prezime su VARCHAR(50) i obavezni su za osnovnu identifikaciju.
uloga je ENUM koji definira funkciju zaposlenika (npr. 'Recepcionist', 'Voditelj', 'Čistačica', 'Održavanje', 'Administrator', 'Nutricionist').
email (VARCHAR(100)) je jedinstven kontakt, dok telefon (VARCHAR(20)) služi kao dodatni kontakt.
datum_zaposlenja je DATE i po defaultu postavlja trenutni datum.
radno_vrijeme je VARCHAR(50) i označava radni raspored zaposlenika.
aktivan je BOOLEAN s default vrijednošću TRUE i označava je li zaposlenik trenutno aktivan.
...
Atribut id je primarni ključ, INT tipa, s AUTO_INCREMENT postavkom. Koristi se za identifikaciju svake transakcije u sustavu.
Atribut id_clana je strani ključ koji se odnosi na člana koji je izvršio uplatu. Povezan je s tablicom clan(id) i osigurava integritet podataka.
Atribut iznos je DECIMAL(6,2) i prikazuje iznos uplate. Obavezan je za unos jer prikazuje stvarno stanje prihoda.
Atribut datum_uplate označava datum kada je uplata izvršena i tipa je DATE. Obavezan je zbog kronološkog praćenja uplata.
Atribut nacin_placanja je ENUM s vrijednostima ('gotovina', 'kartica', 'transfer', 'PayPal', 'kripto'), zadana vrijednost je 'kartica'. Opisuje način plaćanja i koristan je za analizu financija.
Atribut broj_racuna je VARCHAR(20) i jedinstven je (UNIQUE). Koristi se za evidenciju broja računa, ako je primjenjivo.
Atribut popust je DECIMAL(5,2) i predstavlja postotak popusta od 0 do 100, s default vrijednošću 0.00.
Atribut id_osoblje je strani ključ koji označava zaposlenika koji je evidentirao uplatu i povezan je s tablicom osoblje(id).
Atribut opis je tekstualno polje za dodatne napomene.
